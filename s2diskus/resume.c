...
int main(int argc, char *argv[])
{
	...
	ret = mknod(resume_dev_name, S_IFBLK | 0600, makedev(259, 2));
	if(stat(resume_dev_name, &stat_buf)) {
		mprintf("resume device is not ready\n");
		goto Free;
	}
	...
	/* imaged loaded. Now it's up to evs to exit. */
	kill(evs_pid, SIGUSR1);
	if (error) {
		error = -error;
		fprintf(stderr, "%s: Could not read the image\n", my_name);
	} else if (waitpid(evs_pid, &status, 0) != evs_pid) {
	        /* Wait for child (evs) to exit */
		mprintf("wait EVS exit failure\n");
	} else if (freeze(dev)) {
		error = errno;
#ifdef CONFIG_SPLASH
