...
int init_memalloc(size_t aux, size_t pool)
{
	...
	mem = (void*)((unsigned long)mem + slots_size);
	addr = ((unsigned long)mem & ~((unsigned long)page_size - 1));
	if ((unsigned long)addr < (unsigned long)mem)
		mem = (void *)((unsigned long)addr + page_size);
	mem_pool = mem;
	return 0;
}
...
void *getmem(size_t size)
{
	...
	if (cur_slot > slots) {
		...
		addr = (void*)((unsigned long)addr + (((unsigned long)(last_slot->addr) - (unsigned long)mem_pool + last_slot->size +
			align_size - 1) & align_mask));
	}
	used = (size_t)((unsigned long)addr - (unsigned long)mem_pool);
