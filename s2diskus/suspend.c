...
#include "lz4.h"
#include "lz4hc.h"
#define COMPRESS_LZ4_LEVEL	1
#define COMPRESS_FUNC(src, dst, srcSize, dstBound)		LZ4_compress_fast(src, dst, srcSize, dstBound, COMPRESS_LZ4_LEVEL)
//#define COMPRESS_FUNC(src, dst, srcSize, dstBound)		LZ4_compress_HC(src, dst, srcSize, dstBound, 5)
#endif
...
do { \
	fprintf(stderr, "%s: " msg " Reason: %s\n", my_name, ## args, strerror (errno)); \
} while (0)
...
#else
#define compute_checksum 0
#endif
#ifdef CONFIG_COMPRESS
unsigned int compress_buf_size;
static char do_compress = 1;
...
static int save_page(struct swap_writer *handle, void *src)
{
	...
	if (!offset)
		return -ENOSPC;
	//mprintf("save_page src %p offset %d\n", src, (int)offset);
...
#elif (CONFIG_ENCRYPT == ENCRYPT_ISAL)
		mprintf("encrypt_and_save_buffer aes_gcm_enc_128 src = %p, dst = %p\n", src, save_start);
 		aes_gcm_enc_128_update(&key_data.gkey, &key_data.gctx, save_start, src, page_size);
#endif
...
static void save_buffer(struct swap_writer *handle)
{
	...
	while (size > 0) {
		...
		src = (void*)((unsigned long)src + page_size);
		size -= page_size;
	}
}
...
static int encrypt_and_save_page(struct swap_writer *handle, void *src)
{
	...
	if (do_encrypt) {
		...
		if (error)
			return error;
#elif (CONFIG_ENCRYPT == ENCRYPT_ISAL)
		//mprintf("encrypt_and_save_page aes_gcm_enc_128 src = %p, dst = %p\n", src, handle->encrypt_ptr);
		aes_gcm_enc_128_update(&key_data.gkey, &key_data.gctx, handle->encrypt_ptr, src, page_size);
/*
 		if(page_count == 0) {
...
#endif
		src = handle->encrypt_ptr;
		handle->encrypt_ptr = (void*)((unsigned long)handle->encrypt_ptr + page_size);
		if ((unsigned long)handle->encrypt_ptr - (unsigned long)handle->encrypt_buffer
		    >= encrypt_buf_size)
			handle->encrypt_ptr = handle->encrypt_buffer;
	}
...
static int flush_buffer(struct swap_writer *handle)
{
	...
	size = (unsigned long)handle->page_ptr - (unsigned long)handle->buffer;
	if (compute_checksum || verify_image)
	...
	if (do_compress) {
		...
		block->size = cnt;
		//mprintf("lzo1x_1_compress src %p (%d bytes) -> dst %p (%d bytes)\n", handle->buffer, (int)size, block->data, (int)cnt);
		size = cnt + sizeof(size_t);
#elif (CONFIG_COMPRESS == COMPRESS_IGZIP)
		//mprintf("before compression, src %p (%d bytes)\n", handle->buffer, (int)size);
		isal_deflate_stateless_init(&stream);
		stream.flush = FULL_FLUSH;
		stream.level = 1;
		stream.level_buf_size = ISAL_DEF_LVL1_DEFAULT;
		stream.level_buf = handle->lzo_work_buffer;
		stream.avail_in = size;
		stream.end_of_stream = 1;//(uint16_t)end_of_image;
		stream.next_in = handle->buffer;
		stream.avail_out = compress_buf_size;
		stream.next_out = (uint8_t*)block->data;
		error = isal_deflate_stateless(&stream);
		block->size = (unsigned long)stream.next_out - (unsigned long)block->data;
		//mprintf("isal_deflate_stateless src %p (%d bytes) -> dst %p (%d bytes)\n", handle->buffer, (int)size, block->data, (int)block->size);
		if (error != COMP_OK) {
			mprintf("error isal_deflate_stateless ret %d\n", error);
			return error;
		}
		size = block->size+sizeof(size_t);
#elif (CONFIG_COMPRESS == COMPRESS_LZ4)
		error = COMPRESS_FUNC(handle->buffer, block->data, size, compress_buf_size);
		//mprintf("LZ4_compress_fast src %p (%d bytes) -> dst %p (%d bytes)\n", handle->buffer, (int)size, block->data, (int)error);
		if (error <= 0) {
			mprintf("error LZ4_compress_fast ret %d\n", error);
			return error;
		}
		block->size = error;
		size = block->size+sizeof(size_t);
...
static int save_image(struct swap_writer *handle, unsigned int nr_pages)
{
	...
	for (nr_pages = 0; ; nr_pages++) {
		...
		handle->page_ptr = (void*)((unsigned long)handle->page_ptr + page_size);

		if (!(nr_pages % m)) {
			//mprintf("\b\b\b\b%3d%%", nr_pages / m);
#ifdef CONFIG_SPLASH
			...
			while ((key = splash.key_pressed()) > 0) {
				...
				case REBOOT_KEY_CODE:
					mprintf (" reboot enabled\b\b\b\b\b\b\b"
						"\b\b\b\b\b\b\b\b");
		...
		if ((unsigned long)handle->page_ptr - (unsigned long)handle->buffer >= buffer_size) {
...
static int write_image(int snapshot_fd, int resume_fd, int test_fd)
{
	...
	mprintf("System snapshot ready. Preparing to write\n");
	...
	mprintf("Image size: %lu kilobytes\n", (unsigned long) image_size / 1024);
	...
	if (do_compress) {
		...
		handle.swap_needed += round_up_page_size(
					(handle.swap_needed >> 4) + 67); /* TODO: what's this */
	}
...
#elif (CONFIG_ENCRYPT == ENCRYPT_ISAL)
	header->flags |= IMAGE_ENCRYPTED;
	memcpy(header->iv, key_data.ivec, IV_SIZE);
	memcpy(header->key, key_data.key, KEY_SIZE);
#endif
...
Save_image:
	...
	if (!error) {
		...
		aes_gcm_enc_128_finalize(&key_data.gkey, &key_data.gctx, header->tag, TAG_SIZE);
#endif
 ...
 Free_writer:
	...
	if (!error) {
		if (do_compress) {
			mprintf("Compressed size: %lu kilobytes\n", (unsigned long) real_size / 1024);
...
int suspend_system(int snapshot_fd, int resume_fd, int test_fd)
{
	...
	avail_swap = check_free_swap(snapshot_fd);
        mprintf("Available swap size %ld bytes, prefered image size %ld bytes\n", avail_swap, pref_image_size);
	...
	if (test_fd >= 0) {
		mprintf("%s: Running in test mode\n", my_name);
	...
	do {
		...
		if (!in_suspend) {
			...
			mprintf("\e[13]");
			mprintf("%s: returned to userspace\n", my_name);
...
static int prepare_console(int *orig_vc, int *new_vc)
{
	...
	if (!ioctl(fd, TIOCLINUX, tiocl)) {
		...
		suspend_error("TIOCL_GETKMSGREDIRECT error");
        }
...
static void generate_key(void)
{
	...
	rnd_fd = open("/dev/urandom", O_RDONLY);
	if (rnd_fd <= 0)  {
		mprintf("Failed to open urandom to generate key\n");
		return;
	}
...
#elif (CONFIG_COMPRESS == COMPRESS_LZ4)
		compress_buf_size = round_up_page_size(LZ4_compressBound(buffer_size));
		mem_size += compress_buf_size;
#endif
...
/*
        if (prlimit(getpid(), RLIMIT_MEMLOCK, NULL, &old_limit) == -1) {
                suspend_error("Could not get limit.");
        }
        mprintf("Curent limits (%d): soft=%lld; hard=%lld\n",
                   getpid(), (long long) old_limit.rlim_cur, (long long) old_limit.rlim_max);
        new_limit.rlim_cur = 1024*1024;
        new_limit.rlim_max = 1024*1024;
        if (prlimit(getpid(), RLIMIT_MEMLOCK, &new_limit, &old_limit) == -1) {
                suspend_error("Could not set limit.");
        }
        mprintf("New limits (%d): soft=%lld; hard=%lld\n",
                   getpid(), (long long) old_limit.rlim_cur, (long long) old_limit.rlim_max);
*/
